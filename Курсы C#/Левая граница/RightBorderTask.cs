...
namespace Autocomplete
{
    public class RightBorderTask
    {
        ...
        public static int GetRightBorderIndex(IReadOnlyList<string> phrases, string prefix, int left, int right)
        {
            ...
            while (right - 1 != left)
            {
                if (string.Compare(prefix, phrases[(right + left) / 2], StringComparison.OrdinalIgnoreCase) < 0 || phrases[(right + left) / 2].StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
                    left = (right + left) / 2;
                else
                    right = (right + left) / 2;
            }
